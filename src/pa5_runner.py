...
# Thêm **kwargs để nhận các tham số bổ sung (như reference_masks)
def SBOA(N, T, dim, X, y, fitness_func, PopPos_in=None, **kwargs):
    lb, ub = -10, 10
    PopPos = PopPos_in.copy() if PopPos_in is not None else np.random.rand(N, dim) * (ub - lb) + lb
    # Cập nhật cách gọi fitness_func
...
def RBMO(N, T, dim, X, y, fitness_func, PopPos_in=None, epsilon=0.5, **kwargs):
    lb, ub = -10, 10
    PopPos = PopPos_in.copy() if PopPos_in is not None else np.random.rand(N, dim) * (ub - lb) + lb
    pop_fit = np.array([fitness_func(p, X, y, config, **kwargs)[0] for p in PopPos])
...
# Hàm runner nhận thêm **kwargs để chuyển tiếp vào SBOA/RBMO
def run_pa5(X, y, fitness_func, **kwargs):
    dim = X.shape[1]
    N, T, k = config.POP_SIZE // 2, config.MAX_ITER, config.EXCHANGE_INTERVAL
    pop_r, pop_s = None, None
    best_overall_f, best_overall_z = np.inf, None
    ...
    for curr_it in range(0, T, k):
        f_r, pop_r = RBMO(N, k, dim, X, y, fitness_func, PopPos_in=pop_r, **kwargs)
        f_s, pop_s = SBOA(N, k, dim, X, y, fitness_func, PopPos_in=pop_s, **kwargs)

        fit_r = np.array([fitness_func(p, X, y, config, **kwargs)[0] for p in pop_r])
        fit_s = np.array([fitness_func(p, X, y, config, **kwargs)[0] for p in pop_s])
